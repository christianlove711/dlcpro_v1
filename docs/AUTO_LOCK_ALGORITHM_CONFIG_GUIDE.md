...
- `controllers/auto_lock_controller.py`
- `controllers/auto_lock_acquisition.py`
- `controllers/auto_lock_settings.py`
- `controllers/auto_lock_strategies/transmission_primary.py`
- `controllers/auto_lock_strategies/error_primary.py`
- `controllers/auto_lock_strategies/hybrid.py`
- `windows/auto_lock_window.py`
- `widgets/auto_lock/signal_plot.py`
...
这个模式最接近“先确认采集波形中是否存在清晰透射峰”的人工调试习惯：
...
点“加载/切换配置”后，配置会立即同步到自动锁频控制器；点“确定”也会再次应用当前页面参数。
...
- 双路 AD 与协议接入后，检查 FPGA 采样窗口是否和扫频同步。
...
自动锁频现在的核心是：用 PDH 误差信号零交叉和斜率寻找真正可锁点，用透射峰确认该零点确实对应腔共振，再通过 Scan Offset 把零点居中，通过 Scan Amplitude 逐步缩小扫频范围，最后关闭 Scan 并使能 FALC。
